...
// SDA/SCL usan los pines por defecto de Wire en ESP8266: D2 (GPIO4) / D1 (GPIO5).
...
void setup() {
  ...
  // ESP8266: el watchdog de software/hardware se alimenta automáticamente
  // en cada delay()/yield(). Todos los bucles bloqueantes de este sketch
  // ya llaman a delay(1000) o menos en cada vuelta, así que no hace falta
  // alimentarlo a mano. Lo dejamos habilitado explícitamente por claridad.
...
void loop() {
  ESP.wdtFeed(); // alimentar watchdog en cada ciclo
...
void reiniciarRouter() {
  ...
  for (int i = ROUTER_BOOT_TIME_MS / 1000; i > 0; i--) {
    ESP.wdtFeed(); // alimentar watchdog durante la espera larga
...
void esperaLockout() {
  ...
  for (unsigned long i = totalSeconds; i > 0; i--) {
    ESP.wdtFeed(); // alimentar watchdog durante la hora de espera
